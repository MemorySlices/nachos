...
public class KThread {
    ...
    public static void selfTest() {
        System.out.println("Join Test Begin!");
	    Lib.debug(dbgThread, "Enter KThread.selfTest");
        KThread x= new KThread(new TJoin("X")).setName("X");
        KThread y= new KThread(new TBJoin("Y", x)).setName("Y");
        y.fork();
        x.fork();
        ThreadedKernel.alarm.waitUntil(10000);
        System.out.println("Join Test End!");
    }
    private static class TJoin implements Runnable {
        String name;
        TJoin(String nam) {
            name = nam;
        }
    ...
    private static class TBJoin implements Runnable {
        String name;
        KThread X;
        TBJoin(String nam, KThread x) {
            name = nam;
            X = x;
        }
        public void run(){
